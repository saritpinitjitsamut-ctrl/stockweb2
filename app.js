...
try {
    if (!dbSupabase) {
        ...
        // Fallback if auth.js didn't load or initialize
        const URL = 'https://aozzwdaujwuefbkrnbbd.supabase.co';
        const KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFvenp3ZGF1and1ZWZia3JuYmJkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2MDM3NjksImV4cCI6MjA5NjE3OTc2OX0.KkjFlJcyV-Mi_DJE8iIEr28cv84Eq-OXkVhLr20f5es';
        if (window.supabase) {
            dbSupabase = window.supabase.createClient(URL, KEY);
    ...
    async function fetchSkuData() {
        ...
        try {
            const { data, error } = await dbSupabase
                .from('sku_master')
                .select('*, frame_color:colors!frame_color_code(color_name), glass_color:colors!glass_color_code(color_name)')
            ...
            if (data) {
                ...
                const skuName = data.name || data.name_ref || '';
                const skuPattern = data.pattern || data.pattern_ref || '';
                ...
                if (glassEl && glassName) glassEl.value = glassName;

                // เติมประเภทมุ้งจาก net_id
                const screenEl = document.getElementById('screen_type');
                if (screenEl && data.net_id != null) {
                    // net_id 1 = ไม่มีมุ้ง, 2 = มีมุ้ง (ปรับซ้ำมาตรงกับข้อมูลใน mosquito_nets)
                    screenEl.value = data.net_id === 1 ? 'ไม่มีมุ้ง' : 'มีมุ้ง';
                }
    ...
    submitBtn.addEventListener('click', async (e) => {
        ...
        try {
            ...
            const formData = {
                ...
                glass_color: document.getElementById('glass_color') ? document.getElementById('glass_color').value || null : null,
                screen_type: document.getElementById('screen_type') ? document.getElementById('screen_type').value || null : null
            };
    ...
    async function processExcelData(rawData) {
            ...
            try {
                ...
                async function fetchInChunks(column, values) {
                    ...
                    for (let i = 0; i < total; i += chunkSize) {
                        ...
                        try {
                            const selectFields = column === 'product_code'
                                ? '*, frame_color:colors!frame_color_code(color_name), glass_color:colors!glass_color_code(color_name)'
                                : '*';
                ...
                // Fetch by Product Name (รองรับทั้ง column 'name' และ 'name_ref')
                if (namesToFetch.length > 0) {
                    const namesData = await fetchInChunks('name_ref', namesToFetch);
                    namesData.forEach(item => {
                        const itemName = item.name || item.name_ref;
                        if (itemName) {
        ...
        for (let i = 0; i < cleanData.length; i++) {
            ...
            if (master) {
                ...
                // ชื่อสินค้า (รองรับทั้ง name และ name_ref)
                const masterName = master.name || master.name_ref;
                if (masterName) productName = masterName;
                if (master.size) size = master.size;
                
                // ลาย (รองรับทั้ง pattern และ pattern_ref)
                const masterPattern = master.pattern || master.pattern_ref;
            ...
            const aluminumColor = master ? (master.frame_color?.color_name || master.aluminum_color || master.frame_color_code || null) : null;
            const glassColor = master ? (master.glass_color?.color_name || master.glass_color || master.glass_color_code || null) : null;
            const screenType = master ? (master.screen_type || (master.net_id === 1 ? 'ไม่มีมุ้ง' : master.net_id === 2 ? 'มีมุ้ง' : null)) : null;
